...
import SmartThingsStep1 from './SmartThingsStep1.vue';
...
function handleStep2Submit(payload) {
  submitStep2(payload);
}
...
<template>
  <div class="ht-wizard">
    <div class="ht-wizard-header text-center mb-3">
    ...
    <SmartThingsStep1 v-if="currentStep === 1" :config="state.config" @submit="handleStep1Submit" @cancel="goToDashboard" />

    <SmartThingsStep2 v-if="currentStep === 2" :auth-url="state.authUrl" @submit="handleStep2Submit" @back="goBackStep" />

    <SmartThingsStep3 v-if="currentStep === 3" :status="state.status" :error="state.error" @success="handleStep3Success" @retry="retryStep2" @cancel="handleStep3Cancel" />
  </div>
</template>
