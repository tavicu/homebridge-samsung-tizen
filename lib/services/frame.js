...
module.exports = class Frame {
    ...
    createService() {
        if (!this.device.hasOption('Frame.ArtSwitch.Disable')) {
            let service = new Hap.Service.Switch(`${this.device.config.name} Art Mode`, 'frame.art');
        ...
        if (!this.device.hasOption('Frame.PowerSwitch.Disable')) {
            let service = new Hap.Service.Switch(`${this.device.config.name} Power`, 'frame.power');
    ...
    updateValue(value, characteristic, service) {
        service = service || this.services[0];

        if (service) {
            service.getCharacteristic(characteristic || Hap.Characteristic.On).updateValue(value);
        }
    }
