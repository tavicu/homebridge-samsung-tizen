...
module.exports = class Television {
    ...
    createService() {
        const displayOrder = this.accessory.inputs.map(input => input.config.identifier);
    ...
    async refreshValue() {
        this.getActive((error, value) => {
            this.service.updateCharacteristic(Hap.Characteristic.Active, value);
        });
    }
