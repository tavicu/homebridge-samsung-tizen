module.exports = class Inputs {
    constructor(accessory) {
        ...
        this.inputs   = accessory.device.config.inputs;
    ...
    async getInput() {
        ...
        for (let input of this.inputs) {
            ...
            let application = await this.remote.getApplication(input.value);
    ...
    async setInput(value) {
        ...
        if (input.type == 'app') {
            await this.remote.openApplication(input.value);
        } else if (input.type == 'command') {
            await this.remote.command(input.value);
        } else {
