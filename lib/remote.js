...
module.exports = class Remote {
    ...
    async setMain(value) {
        if (this.art) {
            ...
            // If Mode On Enable toggle art mode
            if (this.device.hasOption('Frame.ModeOn.Enable')) {
                newStatus = await this.art.getStatus().then(status => !status);
            }

            return this.setArtMode(newStatus);
        }
