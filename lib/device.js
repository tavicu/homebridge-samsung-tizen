...
module.exports = class Device extends EventEmitter {
    ...
    initRefresh() {
        ...
        this.on('paired', () => {
            let interval = require('./options/refresh')(this.config);

            if (interval.main) {
                setInterval(() => this.mainAccessory.getValue(), interval.main);
            }

            if (interval.switch) {
                let accessories = this.accessories.filter(accessory => accessory.type == 'switch' && !accessory.stateless);

                setInterval(() => accessories.forEach(accessory => accessory.getValue()), interval.switch);
            }
        });
    }
