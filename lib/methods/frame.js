    ...
    getActive(options = {}) {
        return super.getActive().then(status => {
            if (status && options.power !== true) {
                ...
                return this.art.getStatus().then(status => {
                    if (this.device.config.debug) { console.log('active-art', !status); }
                    return !status;
                });
            }
            ...
            if (this.device.config.debug) { console.log('active-final', status, "\n"); }
            return status;
        });
    }
