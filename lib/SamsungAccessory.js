module.exports = class SamsungAccessory {
	...
	if (this.type == 'switch') {
		...
		this.service.getCharacteristic(this.hap.Characteristic.ActiveIdentifier)
		    .on('set', this._setInput.bind(this));
                ...
                let inputSources = new this.hap.Service.InputSource('Sources', 'input0');
                inputSources
                        .setCharacteristic(this.hap.Characteristic.Identifier, 0)
                        .setCharacteristic(this.hap.Characteristic.ConfiguredName, 'Sources')
			...
			.setCharacteristic(this.hap.Characteristic.CurrentVisibilityState, this.hap.Characteristic.CurrentVisibilityState.SHOWN);
                ...
                this.service.addLinkedService(inputSources);


		let inputHDMI = new this.hap.Service.InputSource('HDMI', 'input1');
		inputHDMI
			.setCharacteristic(this.hap.Characteristic.Identifier, 1)
			.setCharacteristic(this.hap.Characteristic.ConfiguredName, 'HDMI')
			.setCharacteristic(this.hap.Characteristic.IsConfigured, this.hap.Characteristic.IsConfigured.CONFIGURED)
			.setCharacteristic(this.hap.Characteristic.InputSourceType, this.hap.Characteristic.InputSourceType.HDMI)
			.setCharacteristic(this.hap.Characteristic.CurrentVisibilityState, this.hap.Characteristic.CurrentVisibilityState.SHOWN);

		this.service.addLinkedService(inputHDMI);


                let inputNetflix = new this.hap.Service.InputSource('Netflix', 'input2');
                inputNetflix
                        .setCharacteristic(this.hap.Characteristic.Identifier, 2)
                        .setCharacteristic(this.hap.Characteristic.ConfiguredName, 'Netflix')
                        .setCharacteristic(this.hap.Characteristic.IsConfigured, this.hap.Characteristic.IsConfigured.CONFIGURED)
                        .setCharacteristic(this.hap.Characteristic.InputSourceType, this.hap.Characteristic.InputSourceType.APPLICATION)
			.setCharacteristic(this.hap.Characteristic.CurrentVisibilityState, this.hap.Characteristic.CurrentVisibilityState.SHOWN);

                this.service.addLinkedService(inputNetflix);
		...
		this.services.push(this.service);
		this.services.push(inputSources);
		this.services.push(inputHDMI);
		this.services.push(inputNetflix);
	...
	this.log(`Input: ${value}`);
		...
		if (value == 0) {
                        let response = await this.remote.sendCmd('KEY_SOURCE');
                        this.log(`Remote key: ${value}, ${response}`);			
		}

                if (value == 1) {
                        let response = await this.remote.sendCmd('KEY_HDMI');
                        this.log(`Remote key: ${value}, ${response}`);
                }

                if (value == 2) {
                        let response = await this.remote.openApp('11101200001');
                        this.log(`Remote key: ${value}, ${response}`);
                }
        }
        catch (error) {
