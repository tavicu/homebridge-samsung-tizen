module.exports = class SamsungAccessory {
    ...
    constructor(device) {
        ...
        this.type    = device.config['type'] || 'power';
        this.time    = device.config['time'];
        this.command = device.config['command'];
        this.channel = device.config['channel'];
        ...
        // Sleep switch
        } else if (this.type == 'sleep' && this.time) {
            this.service.getCharacteristic(this.hap.Characteristic.On)
     ...
     * @param  {Function} callback
     */
    async _getSleep(callback) {
        let status = await this.remote.isOn();

        callback(null, status && this.sleepTime !== null);
    }
     ...
     * @param {Function} callback
     */
    async _setSleep(value, callback) {
        let response;
        ...
        try {
            ...
            if (this.sleepTime && !value ) {
                ...
                return response = 'Timer turned OFF';
            }
            ...
            if (!status) {
                throw 'TV is already OFF';
            }

            // Start the countdown
            this.sleepTime  = setTimeout(async () => {
                // Clear timer
                this.sleepTime = null;

                // Turn the TV OFF
                await this._setOn(false, function() {});
            ...
            }, 1000 * 60 * parseInt(this.time));

            // Set response message
            response = `Timer set to ${this.time} minutes`;
        }
        catch (error) {
            response = error;
